    <nav class="navbar-default navbar-static-side" role="navigation">
        <div class="sidebar-collapse">
            <ul class="nav metismenu" id="side-menu">
                <li class="nav-header">
                    <div class="dropdown profile-element">
                        <img id="Img" class="rounded-circle img-md">
                        <!--<span id="" style="color:white;">
                            <div class="img-container">
                                <div id="outer">
                                    <img id="Img" class="D">
                                </div>
                            </div>
                        </span> -->

                        <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                            <span class="clear"> <span class="block m-t-xs"> <strong class="font-bold" id="NombreUser"> </strong>
                                </span> <span class="text-muted text-xs block"> Opciones <b class="caret"></b></span> </span> </a>
                        <ul class="dropdown-menu animated fadeInRight m-t-xs">
                            <li><a href="#">Perfil</a></li>
                            <li class="divider"></li>
                            <li><a href="#">Ver Perfil</a></li>
                            <li><a href="#">Ver Empresa</a></li>
                            <li><a href="#">Cerrar Sesión</a></li>

                        </ul>
                    </div>
                    <div class="logo-element">
                        <span class="text-center"><img alt="image" src="Logos/LogoIcono.png" style=width:30px;height:30px;" /></span>
                    </div>
                </li>
                <li>
                    <a href="Principal.php"><i class="fa fa-group"></i> <span class="nav-label">Pagina Principal</span></a>
                </li>
                <li>
                    <a><i class="fa fa-th-large"></i> <span class="nav-label">Dashboards</span> <span class="fa arrow"></span></a>
                    <ul class="nav nav-second-level collapse">
                        <li><a href="index.html">Dashboard v.1</a></li>
                        <li><a href="dashboard_2.html">Dashboard v.2</a></li>
                        <li><a href="dashboard_3.html">Dashboard v.3</a></li>
                        <li><a href="dashboard_4_1.html">Dashboard v.4</a></li>
                        <li><a href="dashboard_5.html">Dashboard v.5 </a></li>
                    </ul>
                </li>
            </ul>
        </div>
    </nav>
